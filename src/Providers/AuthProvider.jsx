...
const AuthProvider = ({ children }) => {
  ...
  const signUpNewUser = async (full_name, email, password) => {
    try {
      ...
      const user_availability_in_database =
        await checking_user_availability_in_database(email);
  ...
  const signInExistingUsers = async (email, password) => {
    ...
    try {
      ...
      if (
        userImpression?.loginRestrictedUntil >= currentTimestamp &&
        userImpression?.loginRestricted
      ) {
        const remainingTime =
          userImpression?.loginRestrictedUntil - currentTimestamp;
        ...
        toast.warning(
          `You are currently restricted from logging in. Please try again after ${readableremainingTime}.`
        );
        setUserLoading(false);
        navigate("/");
        return;
      ...
      toast.warning(
        response?.data?.message || `Login failed. Error: ${error.message}`
      );
    } finally {
      setUserLoading(false);
    }
  ...
  const updateExistingUsersPassword = async (oldPassword, newPassword) => {
    try {
      const previousPasswordValidation = await validatePassword(
        auth,
        oldPassword
      );
  ...
  const signInWithGoogle = async () => {
    try {
      ...
      const user_availability_in_database =
        await checking_user_availability_in_database(
      ...
      if (!user_availability_in_database?.exists) {
        ...
        if (response?.status !== 201) {
          new Error(
            "Sign In using Google completed but saving into database failed!"
          );
        }
      }
  ...
  useEffect(() => {
    ...
    const authentication_State_Observer = onAuthStateChanged(
      auth,
      async (currentUser) => {
        ...
        if (currentUser) {
          try {
            ...
            if (tokenResponse?.data)
              console.log(
                "JSON Web Token created and saved. User is logged in."
              );
          } catch (error) {
