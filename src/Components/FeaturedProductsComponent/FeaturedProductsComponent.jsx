...
const FeaturedProductsComponent = () => {
  ...
  const { featuredGadgets } = useSelector(
    (state) => state.featuredGadgetsForHomePage
  );
  ...
  const categories = useMemo(
    () => [
      {
        name: "Smartphones",
        icon: <FaMobileAlt className="mr-2 text-blue-500" />,
      },
      ...
      {
        name: "Projectors",
        icon: <FaProjectDiagram className="mr-2 text-amber-500" />,
      },
    ],
    []
  );
  ...
  const getRandomGadgetsFromEachCategory = useCallback(() => {
    ...
    if (gadgetsData.length !== 0) {
      categoryNames.forEach((categoryName) => {
        const categoryGadgets = gadgetsData.filter(
          (gadget) => gadget?.category === categoryName
        );
        if (categoryGadgets.length > 0) {
          const randomIndex = Math.floor(
            Math.random() * categoryGadgets.length
          );
  ...
  return (
    ...
    >
      ...
      <div className="mb-12 text-center space-y-4">
        <h2
          className={`text-4xl md:text-5xl font-extrabold tracking-tight inline-block transition-all duration-500 ${
            darkMode
        ...
        <p
          className={`text-lg md:text-xl max-w-2xl mx-auto leading-relaxed tracking-wide transition-colors duration-300 ${
            darkMode ? "text-gray-300" : "text-gray-600"
          }`}
        >
          Discover our curated collection of cutting-edge gadgets — all
          available for rent, designed to upgrade your digital lifestyle.
        </p>
      </div>
      ...
      <div className="hidden lg:flex justify-center mb-8">
        <div
          className={`max-w-7xl inline-flex flex-wrap justify-center gap-2 p-2 rounded-2xl ${
            darkMode
